...
module audio_pipeline #(
    ...
    // --- 4. Grain Logic (Phase Accumulation) ---
    genvar k, s;
    generate
            ...
            always @(posedge clk) begin
                if (rst) p_fixed[k] <= 0;
                    ...
                    if (grain_timer == k*STEP)
                        // Reset: Clear fractional bits [8:0] to prevent jitter
    ...
    always @(posedge clk) begin
        ...
        if (rst) begin
            ...
            // Assign BRAM output to the pipe using the delayed index
    ...
    // --- 7. Sequential Accumulator & Scaling ---
